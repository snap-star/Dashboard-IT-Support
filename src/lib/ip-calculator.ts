export function calculateSubnetInfo(network: string, subnetMask: string) {
  ...
  // Validasi format
  if (
    !networkParts.every(part => part >= 0 && part <= 255) ||
  ...
  const networkBinary = networkParts.map(octet => octet.toString(2).padStart(8, '0')).join('')

  const subnetBinary = subnetParts.map(octet => octet.toString(2).padStart(8, '0')).join('')
